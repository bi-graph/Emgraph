...
from emgraph.evaluation.protocol import (
    ParamHistory,
    _flatten_nested_keys,
    _get_param_hash,
    _next_hyperparam,
    _next_hyperparam_random,
    _remove_unused_params,
    _sample_parameters,
    _scalars_into_lists,
    _unflatten_nested_keys,
)
...
def test_evaluate_performance_too_many_entities_warning():
    ...
    with pytest.warns(UserWarning):
        ...
        entities_subset = np.union1d(
            np.unique(X["train"][:, 0]), np.unique(X["train"][:, 2])
        )[:50000]
        evaluate_performance(
            X["test"][::100],
            model,
    ...
    evaluate_performance(
        X_wn18rr["test"][::100], model_wn18, verbose=True, corrupt_side="o"
    )
...
def test_evaluate_performance_filter_without_xtest():
    ...
    X_filter = np.concatenate(
        (X["train"], X["valid"])
    )  # filter does not contain X_test
...
def test_evaluate_performance_ranking_against_shuffled_all_entities():
    ...
    ranks_all = evaluate_performance(
        X["test"][::1000], model, X_filter, verbose=True, corrupt_side="s,o"
    )
...
def test_evaluate_performance_default_protocol_without_filter():
    ...
    ranks_sep = []
    ranks = evaluate_performance(
        wn18["test"][::100], model, verbose=True, corrupt_side="o"
    )
    ...
    ranks = evaluate_performance(
        wn18["test"][::100], model, verbose=True, corrupt_side="s"
    )
    ranks_sep.extend(ranks)
    ...
    ranks = evaluate_performance(
        wn18["test"][::100], model, verbose=True, corrupt_side="s,o"
    )
...
def test_evaluate_performance_default_protocol_with_filter():
    ...
    ranks_sep = []
    ranks = evaluate_performance(
        wn18["test"][::100], model, X_filter, verbose=True, corrupt_side="o"
    )
    ...
    ranks = evaluate_performance(
        wn18["test"][::100], model, X_filter, verbose=True, corrupt_side="s"
    )
    ranks_sep.extend(ranks)
    ...
    ranks = evaluate_performance(
        wn18["test"][::100], model, X_filter, verbose=True, corrupt_side="s,o"
    )
    print("----------corrupted with default protocol-----------------")
    mr_joint = mr_score(ranks)
    mrr_joint = mrr_score(ranks)
...
def test_evaluate_performance_so_side_corruptions_with_filter():
    ...
    ranks = evaluate_performance(
        X["test"][::20], model=model, verbose=True, corrupt_side="s+o"
    )
    mrr = mrr_score(ranks)
...
def test_evaluate_performance_so_side_corruptions_without_filter():
    ...
    X_filter = np.concatenate((X["train"], X["valid"], X["test"]))
    ranks = evaluate_performance(
        X["test"][::20], model, X_filter, verbose=True, corrupt_side="s+o"
    )
    mrr = mrr_score(ranks)
    hits_10 = hits_at_n_score(ranks, n=10)
    print("ranks: %s" % ranks)
...
def test_evaluate_performance_nll_complex():
    ...
    ranks = evaluate_performance(
        X["test"][:200], model=model, filter_triples=filter_triples, verbose=True
    )
...
@pytest.mark.skip(
    reason="Needs to change to account for prime-product evaluation strategy"
)
def test_generate_corruptions_for_eval_filtered():
    ...
    x_n_actual = generate_corruptions_for_eval(
        x, idx_entities=idx_entities, filter=filter_triples
    )
    x_n_expected = np.array([[3, 0, 1], [0, 0, 0], [0, 0, 2], [0, 0, 3]])
    np.testing.assert_array_equal(
        np.sort(x_n_actual, axis=0), np.sort(x_n_expected, axis=0)
    )
...
def test_generate_corruptions_for_eval_filtered_object():
    ...
    x_n_actual = generate_corruptions_for_eval(
        x, idx_entities=idx_entities, filter=filter_triples, side="o"
    )
    x_n_expected = np.array([[0, 0, 0], [0, 0, 2], [0, 0, 3]])
    np.testing.assert_array_equal(
        np.sort(x_n_actual, axis=0), np.sort(x_n_expected, axis=0)
    )
...
def test_generate_corruptions_for_fit_corrupt_side_so():
    ...
    X_corr = generate_corruptions_for_fit(
        dataset, eta=eta, corrupt_side="s,o", entities_size=len(X), rnd=0
    )
    print(X_corr)
...
def test_generate_corruptions_for_fit_curropt_side_s():
    ...
    dataset = tf.constant(X, dtype=tf.int32)
    X_corr = generate_corruptions_for_fit(
        dataset, eta=eta, corrupt_side="s", entities_size=len(X), rnd=0
    )
    print(X_corr)

    # these values occur when seed=0

    X_corr_exp = [[1, 0, 1], [3, 0, 3], [3, 0, 5], [0, 1, 6], [3, 1, 7]]
...
def test_generate_corruptions_for_fit_curropt_side_o():
    ...
    dataset = tf.constant(X, dtype=tf.int32)
    X_corr = generate_corruptions_for_fit(
        dataset, eta=eta, corrupt_side="o", entities_size=len(X), rnd=0
    )
    print(X_corr)
    # these values occur when seed=0

    X_corr_exp = [[0, 0, 1], [2, 0, 3], [4, 0, 3], [1, 1, 0], [0, 1, 3]]
    np.testing.assert_array_equal(X_corr, X_corr_exp)
...
def test_train_test_split():
    ...
    X_train, X_test = train_test_split_no_unseen(
        X, test_size=2, seed=0, backward_compatible=True
    )
...
def test_train_test_split_fast():
    ...
    with pytest.raises(Exception) as e:
        x_train, x_test = train_test_split_no_unseen(
            x_all, 0.99, allow_duplication=False
        )

    assert (
        str(e.value) == "Cannot create a test split of the desired size. "
...
def test_remove_unused_params():
    ...
    assert param["regularizer_params"] == {"p": 1, "lambda": 1e-5}
    assert param["optimizer_params"] == {"lr": 0.001}
...
def test_get_param_hash():
    ...
    params2 = {
        ...
        "loss": "nll",
        "loss_params": {"margin": 2},
        "embedding_model_params": {"useless": 2},
        "regularizer": "LP",
        "regularizer_params": {"p": 1, "lambda": 1e-5},
        "optimizer": "adam",
        "optimizer_params": {"lr": 0.001},
    ...
    params3 = {
        ...
        "embedding_model_params": {"useless": 2},
        "regularizer": "LP",
        "regularizer_params": {"p": 1, "lambda": 1e-4},
        "optimizer": "adam",
        "optimizer_params": {"lr": 0.001},
        "verbose": False,
        "model_name": "ComplEx",
...
def test_sample_hyper_param():
    ...
    param_grid = {
        ...
        "embedding_model_params": {},
        "regularizer": ["LP", None],
        "regularizer_params": {"p": [1, 3], "lambda": [1e-4, 1e-5]},
...
def test_next_hyperparam():
    ...
    assert (
        len(set(frozenset(_flatten_nested_keys(i).items()) for i in combinations))
        == 360
    )
...
def test_select_best_model_ranking_grid():
    ...
    param_grid = {
        ...
        "regularizer_params": {},
        "optimizer": ["adagrad"],
        "optimizer_params": {"lr": [1000.0, 0.0001]},
    }
    ...
    assert set(
        i["model_params"]["optimizer_params"]["lr"] for i in experimental_history
    ) == {1000.0, 0.0001}
    assert (
        len(
            set(
...
def test_select_best_model_ranking_random():
    ...
    param_grid = {
        ...
        "regularizer_params": {},
        "optimizer": ["adagrad"],
        "optimizer_params": {"lr": lambda: np.log(np.random.uniform(1.00001, 1.1))},
    }
    ...
    assert np.all(
        [
            np.log(1.00001)
            <= i["model_params"]["optimizer_params"]["lr"]
            <= np.log(100)
            for i in experimental_history
        ]
    )
