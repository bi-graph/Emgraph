...
def save_model(model, model_name_path=None, protocol=pickle.HIGHEST_PROTOCOL):
    ...
    Examples:
        ...
        >>> from emgraph.utils import save_model
        ...
        >>> example_name = 'emgraph_model.pkl'
...
def restore_model(model_name_path=None):
    ...
    Examples
    --------
        >>> from emgraph.utils import restore_model
        >>> import numpy as np
        >>> example_name = 'emgraph_model.pkl'
...
def create_tensorboard_visualizations(model, loc, labels=None, write_metadata=True, export_tsv_embeddings=True):
    ...
    >>> from emgraph.utils import create_tensorboard_visualizations
    >>>
    >>> X = np.array([['a', 'y', 'b'],
    >>>               ['b', 'y', 'a'],
    >>>               ['a', 'y', 'c'],
...
def dataframe_to_triples(X, schema):
    ...
    >>> import pandas as pd
    >>> import numpy as np
    >>> from emgraph.utils.model_utils import dataframe_to_triples
    >>>
